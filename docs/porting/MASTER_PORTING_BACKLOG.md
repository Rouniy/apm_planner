...
Актуальная поправка пользователя после GeoRef: **сначала востребованные
страницы и инструменты SETUP, начиная с Joystick**, затем реальные остатки
Advanced/Developer и настройки оборудования. Telemetry Logs и дальнейшие
...
результаты и оставшиеся различия — `CURRENT_STATE.md`/`JOYSTICK_PORT.md`.
Текущий inventory129 =79 in-progress/35 partial/15 not-started,
deviations156; SETUP46 страниц, ни одна поверхность не strict-complete.
Следом: **Calibrate Level**, обычные радио/RTK и firmware workflows,
реальные пробелы Advanced/Developer. Не возвращаться к старой очереди
Telemetry Logs/MATLAB из исторических checkpoint ниже.

Предыдущий checkpoint (GeoRef):
...
экспортом не заявляется. Qt5/audio build4 и полный304/304 проходят (52.38s),
production X11 проходит; четыре связанных suites проходят10 повторов. Для
year0001 без GPS-даты исторические часовые пояса Qt/.NET могут различаться;
это отдельное документированное отклонение. DATA теперь8 tabs:
7 mapped+Quick Legacy,8 reference tabs отсутствуют. Deviations151, ни одна
поверхность не strict-complete. Developer32/32 маршрута,
Advanced14 working+1 partial. Исходящие typed MAVLink пакеты теперь записываются
...
Два пробела исходного списка из 11 закрыты: Joystick — теперь встроенная
16-канальная страница с SDL2, профилем и exact-target управлением (`JOYSTICK_PORT.md`),
