...
Проверенная Linux-точка после коммитов `4290221f` и `fb4f08b5`:

- приложение и все цели CMake собираются одним `cmake --build build -j12`;
- проходят 88 из 88 тестов;
...
4. Исправить CONFIG → Onboard OSD: сейчас этот маршрут всё ещё создаёт старый
   legacy `OsdConfig`, тогда как нужен отдельный редактор экранов OSD.
