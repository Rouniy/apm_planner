...
2. Continue the requested SETUP stream from the completed Heli and Frame Type pages; the useful old Compass surface is now explicitly quarantined as `Compass (Legacy)`, and the next complete slice is the native modern MP10 Compass page.
...
The parity inventory currently has 127 product rows: 48 `in-progress`, 42 `partial`, 37 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
- The inherited single-compass page is exposed only as `Compass (Legacy)`. Its useful basic fields remain, invalid manual-declination text is rejected before any write, and unsafe Live/Onboard/CompassMot calibration actions are explicitly disabled until native immutable-target workflows replace them. This does not claim the missing modern Compass or Compass/Motor routes.
