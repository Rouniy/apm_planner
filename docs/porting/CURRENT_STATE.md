...
2. Continue the main `TOOLS` stream: replace the explicitly disabled MP10 entries with complete modeless windows, next with Device Operations now that MAVLink Mirror, NMEA Output and CoT/TAK Output are usable; no visible action may open an empty panel or silently do nothing.
...
The parity inventory currently has 128 product rows: 40 `in-progress`, 46 `partial`, 42 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
The header menu now has the exact 24 MP10 items, order, separators and shortcuts. Eleven routes are enabled because they have a truthful presentation: Developer Tools, Plugin Manager, MAVLink Inspector, MAVLink Mirror, NMEA Output, Cursor-on-Target / TAK Output, Map Tile Cache, Link Statistics, Connection Options, Download Logs (MAVLink) and Tlog Convert / Extract. The other 13 entries are retained in their reference positions but visibly disabled as `not ported yet`; they cannot flip a check mark, open an empty dock or silently no-op. Installed trusted QML tools may append one clearly named extension submenu after the reference inventory.
...
The next principal Tools slice is Device Operations with immutable exact targets, explicit Start/Stop/cancellation and destruction tests. Settings/CONFIG becomes the main stream immediately afterward, as requested.
