...
2. Continue the requested SETUP stream from the completed Heli and Frame Type pages; next split the current unqualified Compass route into a modern MP10 page plus the useful explicitly named legacy workflow.
...
The parity inventory currently has 127 product rows: 48 `in-progress`, 44 `partial`, 35 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
