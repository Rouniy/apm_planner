...
2. Continue the requested `TOOLS` stream after DataFlash Spectrogram; keep each remaining specialized entry in its exact MP10 position but disabled until it has a complete modeless workflow. No visible action may open an empty panel or silently do nothing.
...
The parity inventory currently has 128 product rows: 54 `in-progress`, 39 `partial`, 35 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
- A native Planner Settings page with the exact nine-section MP10 topology, one shared application model across both entry points, live unit/profile/runtime controls, shared live HUD-overlay and speech-policy services, all eight MP10 event gates/templates, exact-target periodic/No Data policy, a bounded serialized TTS queue, a truthful Test Speech status, dual startup UDP policy and an explicitly bounded Legacy settings dialog.
...
The header menu now has the exact 24 MP10 items, order, separators and shortcuts. Thirteen routes are enabled because they have a truthful presentation: Developer Tools, Plugin Manager, MAVLink Inspector, MAVLink Mirror, NMEA Output, Cursor-on-Target / TAK Output, Map Tile Cache, MAVLink Device Operations, Link Statistics, Connection Options, Download Logs (MAVLink), Tlog Convert / Extract and DataFlash Spectrogram. The other 11 entries are retained in their reference positions but visibly disabled as `not ported yet`; they cannot flip a check mark, open an empty dock or silently no-op. Installed trusted QML tools may append one clearly named extension submenu after the reference inventory.
...
`SpectrogramWindow` is a fresh 1050×820 modeless window from both TOOLS/Ctrl+L and SETUP Advanced. It opens Unicode-path DataFlash `.bin`/`.log` files, selects ACC1–5/GYR1–5 and computes separate 1024-point Hann-windowed FFT images for X/Y/Z off the UI thread. A lightweight log-sink interface lets the existing ASCII/binary parsers stream only the chosen sensor instead of retaining the complete log. Direct ACC/GYR rows use fourfold overlap and split on timestamp gaps; current `QHBBHHQf` ISBH plus `QHHaaa` ISBD batches are correlated by number/type/zero-based instance/sequence with incomplete-batch fallback, and the ASCII parser accepts both flattened and bracketed `int16[32]` array fields. Parsing is cancellable, selected input is capped at 2,097,152 samples and each output is capped at 2048×512 by default; when more FFT windows exist than raster columns, per-frequency maximum pooling retains transient peaks. Deterministic direct and synthetic Unicode ASCII/binary batch tests cover axis peaks, the fifth sensor, malformed input, bounds and teardown.
