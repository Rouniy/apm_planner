...
2. Continue the requested SETUP stream from the completed current and legacy Heli pages; next remove the blank-prone combined Frame Type lifecycle and split the useful legacy route from the modern MP10 page.
...
- The complete test suite passes with the Tools catalogue, Link Statistics, Tlog Convert / Extract, MAVLink Mirror, NMEA Output, CoT/TAK, Device Operations, CONFIG Onboard OSD, DisplayView profiles, Planner startup UDP, the native Planner Settings page, shared CONFIG/SETUP MAVFTP and both Heli Setup schemas: **128/128 tests**.
...
The parity inventory currently has 127 product rows: 46 `in-progress`, 46 `partial`, 35 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
The current navigation audit in `SETUP_INVENTORY_AUDIT.md` finds 53 MP10 pages versus 40 concrete Qt page factories. Both have the same three named group headings, for totals of 56 versus 43 navigation entries. Qt is missing 14 MP10 pages and adds the useful native `QML Plugins` manager, so this is intentionally not a one-to-one count. The modern Heli route is first under Mandatory Hardware and uses `H_SW_TYPE`, correcting MP10's stale `H_SWASH_TYPE` gate without aliasing the two schemas. The SETUP `OSD` route uses the dedicated `ConfigHWOSDView` legacy MinimOSD telemetry helper; the separate CONFIG `Onboard OSD` route now uses its own phase-one layout editor, never the unrelated old `OsdConfig`. One shared `displayview` service now applies all 35 MP10 SETUP feature flags to matching existing factories and SETUP persists/restores `setup_lastpage`; missing routes remain missing and useful Qt-only/legacy pages remain honestly classified.
