...
Arm/Disarm and Low Speed now work live through shared application services,
bringing direct MP10 coverage to 20 of 64 controls. Periodic phrases and the
armed No Data warning consume one exact physical-target telemetry epoch; the
TTS backend owns a bounded serialized queue and discards phrases on target
changes. High-priority status messages, vario and speech level remain the next
speech gaps; shortcuts, target-safe rates/identity, map overlays/ADS-B and
