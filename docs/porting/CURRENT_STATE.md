...
2. Continue the requested `TOOLS` stream after DataFlash Spectrogram, 3D Terrain View, External Guided, Follow Me and Moving Base; keep each remaining specialized entry in its exact MP10 position but disabled until it has a complete modeless workflow. No visible action may open an empty panel or silently do nothing. RF Propagation is the next comparatively bounded tool; the five safety-critical Swarm workflows and OSD Video remain disabled until their complete backends exist.
...
The parity inventory currently has 128 product rows: 58 `in-progress`, 38 `partial`, 32 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
The header menu now has the exact 24 MP10 items, order, separators and shortcuts. Seventeen routes are enabled because they have a truthful presentation: Developer Tools, Plugin Manager, MAVLink Inspector, MAVLink Mirror, NMEA Output, Cursor-on-Target / TAK Output, Map Tile Cache, MAVLink Device Operations, Link Statistics, Connection Options, Download Logs (MAVLink), Tlog Convert / Extract, DataFlash Spectrogram, 3D Terrain View, External Guided (File), Follow Me and Moving Base. The other seven entries are retained in their reference positions but visibly disabled as `not ported yet`; they cannot flip a check mark, open an empty dock or silently no-op. Installed trusted QML tools may append one clearly named extension submenu after the reference inventory.
