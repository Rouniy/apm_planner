...
2. Continue the requested SETUP stream from the completed Heli, Frame Type and native current Compass parameter pages; the next coherent Compass slice is the pinned-link onboard/large-vehicle calibration service, followed by the dedicated Compass/Motor route.
...
- A separate real-X11 SETUP smoke used an isolated Copter target with current compass priority/device parameters, clicked the production `Compass` row and rendered its non-empty priority table, use/learn controls, calibration placeholder and advanced fields before the retained Legacy route. The exact-target calibration controls remained visibly disabled with their porting reason; manual Reboot opened the default-Cancel confirmation, Escape canceled it, and the application exited with status 0. Evidence is `/tmp/apm-compass-current-smoke.nnFCm7/compass-current.png` and `/tmp/apm-compass-current-smoke.nnFCm7/reboot-confirmation.png`.
...
The parity inventory currently has 127 product rows: 49 `in-progress`, 42 `partial`, 36 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
The current navigation audit in `SETUP_INVENTORY_AUDIT.md` finds 53 MP10 pages versus 42 concrete Qt page factories. Both have the same three named group headings, for totals of 56 versus 45 navigation entries. Qt is missing 12 MP10 pages and adds the useful native `QML Plugins` manager, so this is intentionally not a one-to-one count. The modern Heli route is first under Mandatory Hardware and uses `H_SW_TYPE`, correcting MP10's stale `H_SWASH_TYPE` gate without aliasing the two schemas; the current and explicitly named legacy Frame Type pages follow it and no longer depend on a late firmware-version callback to create their content. The new current `Compass` route follows Accel Calibration and precedes the explicitly named `Compass (Legacy)` widget; current parameter/priority work is exact-target, while unsafe calibration remains quarantined on both surfaces pending a native service. The SETUP `OSD` route uses the dedicated `ConfigHWOSDView` legacy MinimOSD telemetry helper; the separate CONFIG `Onboard OSD` route now uses its own phase-one layout editor, never the unrelated old `OsdConfig`. One shared `displayview` service preserves all 35 MP10 SETUP feature flags; 30 currently gate matching factories and the five flags for missing Compass/Motor, FFT, Joystick, PX4Flow and REPL routes remain dormant. SETUP persists/restores `setup_lastpage`; missing routes remain missing and useful Qt-only/legacy pages remain honestly classified.
