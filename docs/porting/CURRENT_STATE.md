...
- A native Planner Settings page with the exact nine-section MP10 topology, one shared application model across both entry points, live unit/profile/runtime controls, shared live HUD-overlay and speech-master services, a truthful Test Speech status, dual startup UDP policy and an explicitly bounded Legacy settings dialog.
...
and Test Speech now work live through shared application services, bringing
direct MP10 coverage to 12 of 64 controls. Missing speech event policies/vario,
shortcuts, target-safe rates/identity, map overlays/ADS-B and advanced policies
are stated in place. Useful legacy themes, file paths and seven-rate editing remain in an
