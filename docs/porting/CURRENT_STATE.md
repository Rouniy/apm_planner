...
2. Continue the requested Settings/CONFIG stream from the native Flight Modes and Plane QP Extended Tuning pages; the incomplete controls inside the nine-section Planner page are now the broadest Settings gap.
...
- The complete test suite passes with the Tools catalogue, Link Statistics, Tlog Convert / Extract, MAVLink Mirror, NMEA Output, CoT/TAK, Device Operations, CONFIG Onboard OSD, DisplayView profiles, Planner startup UDP, native HUD/speech Planner controls, the native Planner Settings page, shared CONFIG/SETUP MAVFTP, both Heli Setup schemas, both current/legacy Frame Type pages, the native current Compass parameter/onboard/fixed-yaw workflow, the dedicated Compass/Motor route, shared native Flight Modes and Plane QP Extended Tuning: **142/142 tests**.
...
- A follow-up real-X11 Planner smoke opens the five Speech event controls only after `Enable Speech`, completes the production Mode and Battery configuration prompts, and persists `speechmode="Smoke mode {mode}"`, `speechbattery="Smoke battery {batv} V {batp} percent"`, `speechbatteryvolt=10.5` and `speechbatterypercent=25`. It also verifies that disabling Battery leaves `speechenable=true`, then exits cleanly. Evidence is `/tmp/apm-planner-speech-smoke.PE5xfb/planner-after.png`; the event gates, cancellation and live cross-view synchronization are deterministic in `speechsettings_tests`, `speechannouncer_tests`, `configplannerview_tests` and `batterymonitor_tests`.
...
The parity inventory currently has 127 product rows: 53 `in-progress`, 39 `partial`, 35 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
Test Speech, Armed Only, Waypoint, Mode, Battery and Arm/Disarm now work live
through shared application services, bringing direct MP10 coverage to 17 of 64
controls. The next speech package is periodic Custom, Alt Warning, Low Speed,
No Data/vario and the remaining level/queue behavior; shortcuts, target-safe
rates/identity, map overlays/ADS-B and advanced policies are stated in place.
...
place. The next Settings package completes the remaining Planner speech
consumers and severity policy, followed by the common unit service/speed
controls and then the later full-canvas/tuning-slot OSD
