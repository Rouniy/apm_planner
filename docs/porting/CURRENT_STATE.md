...
2. Continue the requested `TOOLS` stream after DataFlash Spectrogram, 3D Terrain View, External Guided and Follow Me; keep each remaining specialized entry in its exact MP10 position but disabled until it has a complete modeless workflow. No visible action may open an empty panel or silently do nothing. Moving Base is the next staged tool; its store/map foundation is not yet a usable route.
...
The parity inventory currently has 128 product rows: 57 `in-progress`, 38 `partial`, 33 `not-started`, and no row is considered complete yet under the strict visual/cross-platform evidence rule. The duplicate legacy-Heli inventory row has been removed. This means a large amount of global functionality still remains; passing tests does not imply Mission Planner parity.
...
- A native Planner Settings page with the exact nine-section MP10 topology on the canonical CONFIG route, live unit/profile/runtime controls, shared live HUD-overlay and speech-policy services, all eight MP10 event gates/templates, exact-target periodic/No Data policy, a bounded serialized TTS queue, a truthful Test Speech status, dual startup UDP policy and an explicitly bounded Legacy settings dialog. A duplicate standalone settings dialog exists in source but is not user-reachable and is scheduled for removal rather than exposure because MP10 has no second Settings entry.
...
The header menu now has the exact 24 MP10 items, order, separators and shortcuts. Sixteen routes are enabled because they have a truthful presentation: Developer Tools, Plugin Manager, MAVLink Inspector, MAVLink Mirror, NMEA Output, Cursor-on-Target / TAK Output, Map Tile Cache, MAVLink Device Operations, Link Statistics, Connection Options, Download Logs (MAVLink), Tlog Convert / Extract, DataFlash Spectrogram, 3D Terrain View, External Guided (File) and Follow Me. The other eight entries are retained in their reference positions but visibly disabled as `not ported yet`; they cannot flip a check mark, open an empty dock or silently no-op. Installed trusted QML tools may append one clearly named extension submenu after the reference inventory.
...
Moving Base currently has only reusable infrastructure: an endpoint/generation-aware position store with link/reset cleanup and a backend-neutral map API backed by a cyan `BASE` marker in the compiled OPMap/QGCMap implementation. The map presentation deliberately hides exact `(0,0)` while the store preserves it as valid data. The TOOLS action remains disabled because NMEA/serial/TCP/UDP input, settings persistence, bounded diagnostics, Flight Data wiring and complete lifecycle evidence are not yet present; this foundation is not counted as a usable route. MP10 has no position-offset controls, and its legacy Rally Point 0 write will remain visibly unavailable until a current exact-target mission-protocol transaction exists.
...
equivalents. It also found that the standalone `actionSettings` path creates a
real non-empty `ConfigPlannerView` dialog but is orphaned from every visible
header/menu/shortcut; only CONFIG → Planner is production-reachable. MP10 has
no second Settings dialog, so removing this dead duplicate and adding a
production click-through test for every CONFIG row are the first Settings
tasks after the principal TOOLS stream.
...
The verified SETUP packages include the common `ActionPageView`, the exact 16-action `ConfigAdvancedView` inventory with nine working shared tools (MAVLink Inspector, Mavlink Mirror, NMEA, Cursor-on-Target / TAK, DataFlash Spectrogram, External Guided, Follow Me, Map Tile Cache and Proximity), a standalone inspector window, Advanced Terminal, and a user-facing trusted QML plugin manager. Seven Advanced actions remain explicitly disabled. Developer Tools exposes the exact 32-action inventory with working byte/MAVLink/hardware-ID parsers plus the shared Device Operations and 3D Terrain windows: 4 of 32 actions are usable and the other 28 are explicitly disabled. Default Settings, HW ID and ADSB are routed in MP10 order and connected to the committed exact-target parameter snapshot; ADSB identification uses the application-owned exact-link transmitter rather than a legacy global send path.
